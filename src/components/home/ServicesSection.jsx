  ...
  return (
    <section className="services" id="services">
      <div className="container">
        ...
        <div className="service-row">{SERVICES.map((service, index) => <InteractiveServiceCard title={service.label} copy={service.description} index={index} key={service.href} isActive={selectedService === service.label} onSelect={() => setSelectedService(service.label)} />)}</div>
      </div>
      {selectedService && <ProjectInquiryModal service={selectedService} onClose={() => setSelectedService(null)} />}
    </section>
  );
}
